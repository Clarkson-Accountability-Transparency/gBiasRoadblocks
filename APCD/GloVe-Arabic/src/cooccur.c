...
int merge_files(int num) {
    ...
    fwrite(&old, sizeof(CREC), 1, fout);
    fprintf(stderr,"\033[0G         >> Merging Cooccurrence Files: Processed %lld Lines.",++counter);
    for (i=0;i<num;i++) {
        sprintf(filename,"%s_%04d.bin",file_head,i);
        remove(filename);
    }
...
int get_cooccurrence() {
    ...
    if (verbose > 0) {
        fprintf(stderr, "      >> Window Size: %d", window_size);
        if (symmetric == 0) fprintf(stderr, "\n      >> Context: Asymmetric");
        else fprintf(stderr, "\n      >> Context: Symmetric");
    }
    if (verbose > 1) fprintf(stderr, "\n      >> Max Product: %lld", max_product);
    if (verbose > 1) fprintf(stderr, "\n      >> Overflow Length: %lld", overflow_length);
    ...
    if (fid == NULL) {fprintf(stderr,"Unable to Open Vocab File %s.",vocab_file); return 1;}
    ...
    if (verbose > 1) fprintf(stderr,"\033[0G         >> Processed %lld Tokens.\n",counter);
    qsort(cr, ind, sizeof(CREC), compare_crec);
    write_chunk(cr,ind,foverflow);
