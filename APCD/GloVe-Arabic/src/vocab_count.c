...
int get_counts() {
    ...
    while (fscanf(fid, format, str) != EOF) { // Insert all tokens into hashtable
        if (str[0] == '\0') continue; // Skip empty strings
        if (strcmp(str, "<unk>") == 0) {
    ...
    if (verbose > 1) fprintf(stderr, "\033[0G      >> GloVe Processed %lld Tokens.\n", i);
    ...
    if (verbose > 1) fprintf(stderr, "\033[0G      >> GloVe Counted %lld Unique Words.\n", j);
    ...
    fprintf(stderr, "\033[0G      >> GloVe Vocabulary Size %lld.\n", i);
    return 0;
}
